...
service like any other, and losim ships its schema.
...
no socket. The simulation places files on nodes; losim builds every server and
...
— none of which losim models, because nodes are named and found by what they
serve.
...
| | |
|---|---|
| **Build no channel or server** | `ManagedChannelBuilder`, `Grpc.newChannelBuilder`, `ServerBuilder.forPort`. A peer is found by what it serves: `Losim.current().peersServing(name)` and `Losim.current().channelTo(peer)`. A channel you built carries no interceptor, so its calls are absent from the wire, the bill and the film. |
...
| **Do not sleep in host milliseconds** | `Thread.sleep` is the one duration the compressed clock never touches, so at a compression of forty it is forty times too long. `Losim.current().sleep(refMs)` is reference time, like every other duration. |
...
| **Hold no mutable static state** | a static collection shared by every node is one node pretending to be many. Instance fields are per-node and are what a node "remembers"; `Losim.current().local()` is the store every service on one node shares, and a restart empties it. |
...
| **One class per file, named after it** | a `runs:` value is a path, and the class it loads is the one that file is named after. A nested service has no line anybody could write, so it is refused. losim builds a fresh instance when a node restarts, so it needs a public no-argument constructor. |
...
The client is the thing that drives calls. That is what `losim.Job` is, and its
two rpcs are the split between reading your input and doing the work:

```java
public final class MyJob extends losim.pb.JobGrpc.JobImplBase {
    ...
    @Override public void load(losim.pb.Input in, StreamObserver<losim.pb.Workload> out) {
        Frames frames = build(in.getCount(), losim.api.Losim.current().seed());
        out.onNext(losim.pb.Workload.newBuilder()
    ...
    @Override public void run(losim.pb.Workload work, StreamObserver<losim.pb.Result> out) {
        var here = losim.api.Losim.current();
        ...
        out.onNext(losim.pb.Result.newBuilder()
...
and `shutdownNow()`. Name the file in the `losim.Job` entry of some node's
...
`Losim.current().units(n)` in it and give the rpc a `perUnit:`.
...
`unit:` is the same word `Losim.current().units(n)` counts and `perUnit:` prices.
Leave `source:` out and `Load` generates the workload from
`Losim.current().seed()` — reproducible from the seed, different across a sweep,
...
Optional. Nothing in losim requires it. Note that the abstract base is not what a
