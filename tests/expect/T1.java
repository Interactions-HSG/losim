...
public final class T1 {
    public static void main(String[] args) {
        ...
        e.check(!Dissaly.current().isRunning(),
                "and it knows nothing is running, rather than being told otherwise");

        boolean silent = true;
        try {
            Dissaly.current().reveal("emitted", 2);
            Dissaly.current().units(1);
        ...
        String why = null;
        try { Dissaly.current().peers(); } catch (IllegalStateException x) { why = x.getMessage(); }
        ...
        String dialled = null;
        try { Dissaly.current().channelTo("srv"); } catch (IllegalStateException x) { dialled = x.getMessage(); }
