 ...
 *
 * Recording calls are silent, so a unit test need not depend on dissaly. State calls
 * throw because an invented cluster or clock would make the test misleading.
 */
final class Absent implements DissalyCtx {
    ...
    private static IllegalStateException absent(String call) {
        return new IllegalStateException(
                "no simulation is running, so Dissaly.current()." + call + " has no answer here. "
