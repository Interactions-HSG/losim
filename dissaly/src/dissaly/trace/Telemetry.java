...
public final class Telemetry {
    ...
    public static final Context.Key<Span> SPAN = Context.key("dissaly.span");
    ...
    public static final class Span {
        ...
        /** Wall clock spent inside this span on dissaly's own bookkeeping, not the program's. */
        public final AtomicLong dissalyNanos = new AtomicLong();
        /** Bytes allocated inside this span by dissaly's own bookkeeping, not the program's. */
        public final AtomicLong dissalyBytes = new AtomicLong();
        /**
         * Bytes the <i>handler</i> allocated, once dissaly's own are taken back off.
         *
        ...
        /** Open to close, including whatever dissaly did meanwhile. */
        public double grossMs() { return t1 < 0 ? -1 : t1 - t0; }

        /**
         * What the <i>program</i> took.
         *
         * <p>dissaly's own work runs inside this window, on this thread, so leaving
        ...
        public double programMs(double kTime) {
            if (t1 < 0) return -1;
            return Math.max(0, (t1 - t0) - dissalyNanos.get() / 1e6 * kTime);
        }
    }
     ...
     *
     * <p>For {@code dissaly.Job/Load}, which runs against a system that is up and
