...
import dissaly.time.Dispatcher;
 ...
 * against what the system really serves; then {@code dissaly.Job/Load}, off the
 ...
 * the sampler; and only then {@code dissaly.Job/Run}.
 *
...
public final class Simulate {
    ...
    public record Result(Trace trace, Telemetry telemetry, boolean completed,
        ...
        public record Totals(String name, long peakRetainedBytes, long allocatedBytes,
                             ...
                             long handledCalls, long dissalyBytes, long dissalyStops,
    ...
    private static Clock.Calibration calibrate() {
        ...
        if (!c.usable()) {
            ...
            System.err.printf(
                    "dissaly: this host's timer measured %.2f (ordinarily about 1.28)%s."
    ...
    public static Result of(Simulation s, ClassLoader loader, Telemetry.Level level, Trust trust)
        ...
        // billed for it. Paid here, on a cluster of dissaly's own, in no trace.
        Warm.once();
        ...
        try (var machines = new Machines(tel, net, s.seed())) {
            ...
            try (var outside = new Outside(entry.name)) {
                ...
                throw new IllegalArgumentException("dissaly.Job/Load failed on " + entry.name
            ...
            try (var outside = new Outside(entry.name)) {
                ...
                dissaly.pb.Result answer = call.get(WATCHDOG_SECONDS, TimeUnit.SECONDS);
                completed = true;
                ...
                if (!entry.alive() || entry.restarts() > 0)
                    failure += " — " + entry.name + " runs dissaly.Job and was restarted, which"
                            + " takes its server down and cancels the call it was serving";
                tel.event(entry.name, "failed", "error", failure);
            } catch (TimeoutException e) {
                failure = "dissaly.Job/Run was still running after " + WATCHDOG_SECONDS
            ...
            for (Machine m : machines.all()) {
                ...
                totals.put(m.name(), new Result.Totals(m.name(), m.peakRetainedBytes(),
                        ...
                        m.crossZoneBytes(), m.handledCalls(), m.dissalyBytes(), m.dissalyStops(),
            ...
            for (Machine machine : machines.all()) {
                ...
                // What dissaly itself cost this machine, and how often it stopped to
                ...
                m.put("dissalyMb", mb(t.dissalyBytes()));
                m.put("dissalyStops", t.dissalyStops());
    ...
    private static Supplier<BindableService> factory(String className, ClassLoader loader,
        ...
        try { type.getDeclaredConstructor(); }
            ...
            throw new IllegalArgumentException(where + ": '" + className + "' needs a"
                    + " no-argument constructor, because dissaly builds a fresh one when a machine"
                    + " restarts. Whatever it needs, it can ask Dissaly.current() for.");
        }
        return () -> {
            try {
