 ...
 * machines run on real threads against a real wall clock, and dissaly shapes what
 ...
 * marshaling, real allocation, real contention. dissaly is the layer on top that
...
public final class Machines implements AutoCloseable {
    ...
    static final Metadata.Key<String> PARENT =
            Metadata.Key.of("dissaly-parent-span", Metadata.ASCII_STRING_MARSHALLER);

    /**
     * Marks the one call nobody in the system made: dissaly's own, to
     * {@code dissaly.Job}.
     *
    ...
    static final Metadata.Key<String> OUTSIDE =
            Metadata.Key.of("dissaly-outside", Metadata.ASCII_STRING_MARSHALLER);
    ...
    /**
     * Which of {@code dissaly.Job}'s rpcs this simulation has already been entered
     * through.
     *
    ...
    ExecutorService waiting() {
        ...
        if (w == null) synchronized (this) {
            ...
            if (w == null) waiting = w = Executors.newCachedThreadPool(r -> {
                Thread t = new Thread(r, "dissaly-waiting");
                t.setDaemon(true);
                return t;
            });
        }
