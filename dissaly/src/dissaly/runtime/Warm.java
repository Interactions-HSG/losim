 ...
 *
 * <p>So it is paid here, once per JVM, on a throwaway cluster of dissaly's own that
 * appears in no trace, before {@link Machines#begin()} zeroes the clock. Setup
 * belongs to no simulation.
 *
 * <p><b>What it does not fix.</b> This warms dissaly's path, not the student's
 ...
 * fabricated request, before their job has started, is not something dissaly may
...
final class Warm {
     ...
     *
     * <p>{@code Empty} rather than a dissaly-specific message, so no {@code .proto}
     * of dissaly's own has to be generated and shipped to make this work.
     */
    private static final MethodDescriptor<Empty, Empty> TOUCH =
            MethodDescriptor.<Empty, Empty>newBuilder()
                    .setType(MethodDescriptor.MethodType.UNARY)
                    .setFullMethodName("dissaly.Warm/Touch")
    ...
    private static void warm() throws Exception {
        ...
        try (var machines = new Machines(tel, net)) {
            BindableService touch = () -> ServerServiceDefinition.builder("dissaly.Warm")
                    ...
                    .build();
            Machine caller = machines.machine("dissaly-warm-a", "m5.large", "eu-central-1a");
            machines.machine("dissaly-warm-b", "m5.large", "eu-central-1a").serving(touch);
            caller.serving();
            machines.begin();
            for (int i = 0; i < ROUNDS; i++) {
                ...
                // to cost nothing.
                ManagedChannel warm = caller.channelTo("dissaly-warm-b");
                try {
