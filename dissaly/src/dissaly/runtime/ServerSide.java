...
/**
 * What dissaly does around a call the machine is serving: the declared cost, the
 * ambient context, and the record of what happened.
 *
 * <h2>Everything between the marks is dissaly's</h2>
 ...
 * dissaly's own work is bracketed and charged to the machine's ledger, which
 * {@link Machine#allocatedBytes()} then subtracts (D13). gRPC's own work stays
 * outside the marks, because it is not dissaly's to give back.
 *
 * <p>Two things are pointedly <b>not</b> bracketed: the cost sleep and the
 * per-unit sleep. Those are the simulated program's time, not dissaly's, and
...
final class ServerSide implements ServerInterceptor {
    ...
    @Override public <Q, S> ServerCall.Listener<Q> interceptCall(
        ...
        // dissaly.Job is dissaly's own way into the system, and it is used once.
        //
        // Neither half of that is checkable at load: a node can only make this call
        // while running, and the header that marks dissaly's own call can be attached
        ...
        if (full.startsWith(dissaly.pb.JobGrpc.SERVICE_NAME + "/")) {
            String no = null;
            if (headers.get(Machines.OUTSIDE) == null)
                no = method + " is dissaly's call into the system, and nothing inside the"
                ...
                no = method + " has already been called. dissaly loads once and runs once,"
        ...
        var listener = new ForwardingServerCallListener
            ...
            @Override public void onHalfClose() {
                ...
                // some of dissaly's own work — `sendMessage` and `close` are both
                ...
                // other direction.
                long h0 = Meter.allocNow(), l0 = span.dissalyBytes.get();
                try {
                    super.onHalfClose();
                } finally {
                    long mine = Meter.allocNow() - h0;
                    long ours = span.dissalyBytes.get() - l0;
