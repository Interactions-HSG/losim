...
public final class Bills {
        ...
        if (both.projected() != null) {
            ...
            if (!both.projected().complete())
                System.out.println("""
                      ...
                      which is the noisiest thing dissaly measures — so the uncertainty in what
    ...
    /** The price list named, the one dissaly ships under that name, or the defaults. */
    private static PriceList pricesFor(String priceFile) throws Exception {
        ...
        // No file, but this may be a region dissaly ships. A lab carries no copy of
        // the price lists — it resolves dissaly from Maven — and billing it at the
        ...
        System.err.println("no price list at " + priceFile
                + " and none of that name inside dissaly; using the built-in defaults");
