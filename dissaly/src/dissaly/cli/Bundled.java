 ...
 * instead, so that depending on dissaly is the whole of getting dissaly: one
 ...
 * manage, two processes to race over it — {@code dissaly serve} and {@code dissaly
...
final class Bundled {
    ...
    static Path dir(String name, String marker) {
        URL found = Bundled.class.getResource("/dissaly/" + name + "/" + marker);
        if (found == null) return null;
        try {
            URI uri = found.toURI();
            // Running off build/classes rather than a jar, which is how dissaly's own
            ...
            // jar:file:/…/dissaly.jar!/dissaly/docs/docs.json — the filesystem is the
            ...
            return fs.getPath("/dissaly/" + name);
        } catch (Exception e) {
