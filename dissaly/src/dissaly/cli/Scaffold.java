 ...
 *
 * <p><b>None of these is a {@code .java}.</b> dissaly writes its own furniture and
 ...
 * turning a client into a dissaly.Job implementation is a judgement about which of its
...
final class Scaffold {
    ...
    /** Where a lab resolves dissaly from, anonymously and with nothing to configure. */
     ...
     * protobuf-gradle-plugin: dissaly generates into {@code gen/} beside the schema
     ...
     * Portal resolution and skew its protoc against dissaly's. It does not set a
    ...
    static String build(String grpc, String protobuf) {
        ...
        return """
            ...
            repositories {
                mavenCentral()
                // dissaly itself. A plain file tree over HTTPS, so a lab needs no
                // credential and nothing to configure — see docs/start/adopt.
                maven { url = uri("%s") }
            }

            val dissalyVersion = "%s"
            val protobufVersion = "%s"
            val grpcVersion = "%s"

            dependencies {
                // gRPC and protobuf arrive with it: dissaly declares them as `api`
                // dependencies because your handlers import them directly.
                implementation("%s:dissaly:$dissalyVersion")
            }
            ...
            // constraint that was actually meant and is what dissaly itself uses.
            ...
            // the protobuf sources dissaly generates beside the schema rather than off
            ...
            // ------------------------------------------------------------- the toolchain
            //
            // What dissaly reads instead of going looking: the build's own resolved
            ...
            // dissaly generates by reading this file. A task that cannot run until it has
            ...
            val dissalyToolchain by tasks.registering {
                ...
                val bin = layout.buildDirectory.dir("dissaly/bin")
                val line = layout.buildDirectory.file("dissaly/classpath")
    ...
    static String launcher() {
        return """
            #!/usr/bin/env sh
            # dissaly, in this project. `./dissaly simulate simulations/1-one-call.yaml`, `./dissaly serve`.
            #
     ...
     *
     * @param entry the {@code .java} implementing {@code dissaly.Job}, which is where
     *              dissaly enters the system. Usually a file that does not exist yet:
    ...
    static String simulation(String entry, List<String[]> runs, List<String[]> costs) {
        ...
        sb.append("""
            ...
            nodes:
              # dissaly calls Job.Load here off the clock, and then Job.Run. That second
        ...
        sb.append("""
            ...
            # What dissaly hands Job.Load, before the clock starts. `count:` is the one
            # number the engine varies: at scale: 1 it is what you wrote, and above it
            # dissaly shrinks it, measures what comes back and fits the rest.
            #
            # `unit:` is what one of them is called, singular — frame, line, order. It
            # is the same word Dissaly.current().units(n) counts and perUnit: prices.
            #
            # Add `source: data/whatever` to read the workload from a file or a folder.
            # Left out, as here, Load generates it from Dissaly.current().seed(), which
    ...
    static String gitignore() {
        return """
            # dissaly generates protobuf sources beside the schema, where the editor finds
