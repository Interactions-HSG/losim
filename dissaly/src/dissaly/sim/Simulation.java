...
public record Simulation(
         ...
         * dissaly nowhere: a system can be written, compiled and unit-tested with
     ...
     *
     * <p><b>Not the node that runs {@code dissaly.Job}.</b> That one starts the work
    ...
    public int workers() {
        return (int) Math.max(1, nodes.stream()
                ...
                .filter(m -> !m.runs().containsKey(dissaly.pb.JobGrpc.SERVICE_NAME))
                .count());
    }
     ...
     *               with dissaly.jar alone; see {@link Codes}. The runtime resolves
