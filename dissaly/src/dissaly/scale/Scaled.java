...
public final class Scaled {
    ...
    public static Result of(Simulation s, ClassLoader loader, Telemetry.Level level,
        ...
        for (var e : probe.resources().entrySet()) {
            ...
            // The rule lives in `Laws.reported` and is asked for here rather than
            // repeated, because the notes underneath this table have to answer the
            // same question and two copies of it drifted apart once already.
            boolean assembled = false;
            // A refused resource stays refused. Assembling one from its parts
            // overwrites the reason with a number, and this shipped: a memory law
            // refused for bending came back as `projected: 0` with the refusal
            // cleared, which is a refusal turned into a confident answer — the one
            // outcome the whole design exists to prevent.
            if (p.projected().isPresent()) {
                var reported = plan.laws().reported(e.getKey(), plan.fullUnits());
                if (reported.assembled()) {
                    p = new ScalePlan.Projection(p.resource(), p.observed(), reported.value(),
