...
public class Phase2 {
    ...
    static String workers(String extra) {
        return "nodes:\n"
             + "  master: { instance: m5.large, zone: z, runs: { dissaly.Job: dissaly/test/src/WaitJob.java } }\n"
             + "  workers:\n"
             + "    count: 2\n"
             + "    prefix: w\n"
             + "    instance: m5.large\n"
             + "    zone: z\n"
             + "    runs: { Volley: dissaly/test/src/Pinger.java }\n"
             + extra
             + "simulatedDuration:\n"
             + "  dissaly/test/src/Pinger.java: { Hit: { fixed: 1 refMs } }\n";
    }

    static final String CLUSTER = """
        nodes:
          master: { instance: m5.large, zone: z, runs: { dissaly.Job: dissaly/test/src/NoopJob.java } }
          workers: { count: 2, prefix: w, instance: m5.large, zone: z, runs: { Volley: dissaly/test/src/Pinger.java } }
        simulatedDuration:
          dissaly/test/src/Pinger.java: { Hit: { fixed: 1 refMs } }
        """;
     ...
     *
     * <p>Machines are found by what they serve and called over a channel dissaly made,
     ...
     * without a channel dissaly made, the only way to fan out would be a channel of
    ...
    static void forwarding() throws Exception {
        ...
        var result = Simulate.of(Loader.of(Yaml.parse("forward.yaml", """
            ...
            nodes:
              master: { instance: m5.large, zone: z, runs: { dissaly.Job: dissaly/test/src/ForwardJob.java } }
              front:  { instance: m5.large, zone: z, runs: { Worker: dissaly/test/src/Forwarder.java } }
              back:   { instance: m5.large, zone: z, runs: { Worker: dissaly/test/src/Counter.java } }
            simulatedDuration:
              dissaly/test/src/Forwarder.java: { Map: { fixed: 1 refMs } }
              dissaly/test/src/Counter.java:   { Map: { fixed: 15 refMs }, Reduce: { fixed: 100 refMs } }
            """)));
    ...
    static void refusals() {
        ...
        refuses("a file whose class is not named after it",
                """
                nodes:
                  w0: { instance: m5.large, zone: z, runs: { Worker: dissaly/test/src/NotItsOwnName.java } }
                """,
    ...
    static void retryGate() throws Exception {
        ...
        var result = Simulate.of(Loader.of(Yaml.parse("simulation.yaml", """
                nodes:
                  master: { instance: m5.large, zone: z, runs: { dissaly.Job: dissaly/test/src/RetryJob.java } }
                  workers: { count: 1, prefix: w, instance: m5.large, zone: z, runs: { Volley: dissaly/test/src/Pinger.java } }
                retries:
    ...
    static String badRpc(String rules) {
        return "seed: 4\n"
             ...
             + "    runs:\n"
             + "      Volley:\n"
             + "        file: dissaly/test/src/Pinger.java\n"
             + "        failures:\n"
             + rules
             + "simulatedDuration:\n"
             + "  dissaly/test/src/Pinger.java: { Hit: { fixed: 1 refMs } }\n";
    ...
    static void rpcFailures() throws Exception {
        ...
        try {
            ...
            // of its own. A refusal that prints it back as 'dissaly/test/src/Volley
            // java Hitt' names no file the reader can open.
            readable = said.contains("dissaly/test/src/Pinger.java Hitt");
    ...
    static void disk() throws Exception {
        ...
        var result = Simulate.of(Loader.of(Yaml.parse("simulation.yaml", """
                nodes:
                  master: { instance: m5.large, zone: z, runs: { dissaly.Job: dissaly/test/src/WordCountJob.java } }
                  workers:
                    count: 2
                    prefix: w
                    instance: m5.large
