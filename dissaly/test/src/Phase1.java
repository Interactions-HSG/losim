...
public class Phase1 {
    ...
    public static void main(String[] args) throws Exception {
        System.out.println("Phase 1 — one in-process server per machine, dissaly as interceptors\n");
    ...
    static void exclusion() throws Exception {
        System.out.println("=== what dissaly gives back, and what it cannot ===");
    ...
    static Ladder ladder(int[] sizes, long[] seeds, int reveals) throws Exception {
        ...
        for (int i = 0; i < sizes.length; i++) {
            ...
            regions = median(runs.stream().map(r -> (double) r.dissalyStops()).toList());
        }
    ...
    static void observerLaw() throws Exception {
        ...
        System.out.println("  A ladder is fitted twice: once with no dissaly inside the handler at all,");
        ...
        System.out.printf("  %-12s %10.4f %10s   %s%n", "keys", keyBend, "-",
                "the control: nothing dissaly does can move it");
        ...
        check(median(heavy.stream().map(Ladder::regions).toList())
              ...
              "heavier dissaly use is metered as more regions, not estimated as a per-call constant");
