...
/**
 * dissaly.Job is dissaly's way in, and it is used once.
 *
 * <p>Neither half is checkable when the file is read: a node can only make this
 * call while running, and the header marking dissaly's own call is one line of
...
class EntryOnceTest {
    ...
    /** The header dissaly puts on its own call, spelled the way anyone could spell it. */
    ...
    void twiceIsRefused() {
        try (var machines = cluster()) {
            ...
            assertEquals("yes", stub.run(Workload.getDefaultInstance())
                    ...
                    "a call carrying the header is dissaly's as far as anyone can tell");
