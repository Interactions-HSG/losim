 ...
 *
 * <p>A lab resolves dissaly with Gradle, so its jars sit in a package cache under
 * names and versions the build chose. There is nowhere for dissaly to go looking,
 ...
 * silent: a lab dissaly declines to recognise shows a student an empty console with
 * no error in it.
 */
class LabToolchainTest {
     ...
     *
     * <p>Every classpath here would otherwise name nothing real, which dissaly
    ...
    void declaredIsALab(@TempDir Path root) throws Exception {
        String cp = jar(root, "dissaly.jar") + java.io.File.pathSeparator + jar(root, "grpc.jar");
    ...
    void foreignClasspath(@TempDir Path root) throws Exception {
        ...
        declare(root, "classpath=/Users/someone/build/dissaly.jar:/Users/someone/.gradle/grpc.jar\n");
        Lab lab = at(root);
    ...
    void saysWhatItIgnored(@TempDir Path root) throws Exception {
        ...
        declare(root, "classpath=/Users/someone/dissaly.jar\nprotoc=/Users/someone/protoc-osx-aarch_64\n");
    ...
    void quietWhenHonoured(@TempDir Path root) throws Exception {
        ...
        declare(root, "classpath=" + jar(root, "dissaly.jar") + "\n");
