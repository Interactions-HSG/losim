...
/**
 * The same system, served and called through gRPC, with dissaly not running.
 *
 ...
 * compressed clock and no interceptor, and dissaly's runtime is never started —
 ...
 * dissaly system is a plain gRPC system, which is what makes the VS Code Java
 ...
 * to write down and easy to break — one dissaly type in a signature, one static
...
class GrpcAloneTest {
    ...
    @Test
    @DisplayName("dissaly is on the classpath and is not running")
    void nothingIsSimulated() {
        // dissaly.jar is here because Counter calls Dissaly.current(), which has to
